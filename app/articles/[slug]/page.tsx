...
import { formatDate, calculateReadingTime, getRelatedArticles } from '@/utils/articleHelpers';
      ...
      <main className="relative pt-32 pb-12 sm:pt-28 sm:pb-10 min-h-screen z-0">
        ...
        <article className="container mx-auto px-4 max-w-4xl">
          ...
          <header className="mb-10">
            ...
            {/* Title */}
            <h1 className="font-lexend text-5xl sm:text-3xl font-bold text-text mb-6 leading-tight">
              {article.title || article.metadata.title}
            </h1>

            {/* Meta Information */}
            <div className="flex flex-wrap items-center gap-6 mb-6 pb-6 border-b border-[#f1f1f1]">
              <div className="flex items-center gap-2 font-nunito text-sm text-text-light">
                <LuUser className="text-lg" />
                <span>{article.author?.name || 'Interview Screener Team'}</span>
              </div>
              <div className="flex items-center gap-2 font-nunito text-sm text-text-light">
                <LuCalendar className="text-lg" />
                <span>{formatDate(article.publishedAt || article.createdAt || article.updatedAt)}</span>
              </div>
              <div className="flex items-center gap-2 font-nunito text-sm text-text-light">
                <LuClock5 className="text-lg" />
                <span>{readingTime} min read</span>
              </div>
            </div>

            {/* Social Share */}
            <SocialShare title={article.title || article.metadata.title} url={articleUrl} />
          </header>
          ...
          {article.tags && article.tags.length > 0 && (
            <div className="mb-12 pb-12 border-b border-[#f1f1f1]">
              <div className="flex flex-wrap gap-2">
                {article.tags.map((tag, index) => (
                  <span
                    key={index}
                    className="px-3 py-1.5 bg-[#f8f8f8] text-text-light rounded-lg font-nunito text-sm"
                  >
                    #{tag}
                  </span>
                ))}
          ...
          <div className="mb-12 pb-12 border-b border-[#f1f1f1]">
            <p className="font-nunito text-text mb-4">Found this article helpful?</p>
            <SocialShare title={article.title || article.metadata.title} url={articleUrl} />
          </div>
        </article>
 ...
 * Set to 0 to disable caching and always fetch latest article data
 */
