  ...
  return (
    <header
      className={`
        ...
        ${isHide ? 'top-[-100%] opacity-0' : ''}
        
        /* Mobile styles (Base) */
        top-0 left-0 w-full rounded-none px-4 py-3
        
        /* Desktop styles (LG+) - Floating Container */
        lg:top-5 lg:left-1/2 lg:-translate-x-1/2 lg:w-[90%] lg:max-w-[1080px] lg:rounded-[14px] lg:px-5 lg:py-2.5
      `}
    >
      <nav className="flex items-center justify-between max-w-full">
            ...
            {navLinks.map((link) => (
              link.isSection ? (
                <a
                  ...
                  className="relative px-3 py-2 text-[15px] font-medium text-[#1E1E1E] transition-colors hover:text-primary group whitespace-nowrap"
                >
                  {link.label}
                ...
                <Link
                  ...
                  href={link.href}
                  className={`
                    relative px-3 py-2 text-[15px] font-medium transition-colors hover:text-primary group whitespace-nowrap
                    ${pathname === link.href ? 'text-primary' : 'text-[#1E1E1E]'}
                  `}
                >
                  {link.label}
                  <span 
          ...
          <div className="hidden lg:flex items-center gap-2 flex-shrink-0">
            <Link
              ...
              className="px-4 py-1.5 text-sm rounded-md border border-primary text-primary font-medium hover:bg-primary hover:text-white transition-colors whitespace-nowrap"
            >
              Log In
            </Link>
            <Link
              href="https://app.interviewscreener.com/sign-up"
              className="px-4 py-1.5 text-sm rounded-[8px] bg-gradient-to-t from-[#00A0E2] via-[#3388FF] to-[#3388FF] text-white font-medium hover:opacity-90 transition-opacity whitespace-nowrap"
            >
              Sign Up
            </Link>
          </div>
        ...
        >
          <div className="flex flex-col gap-4 pb-4">
            {navLinks.map((link) => (
              link.isSection ? (
                <a
                  ...
                  href={link.href}
                  onClick={(e) => handleSectionClick(e, link.href)}
                  className="text-[17px] font-medium text-[#1E1E1E] hover:text-primary"
                >
                  {link.label}
                </a>
              ) : (
                <Link
                  key={link.label}
                  href={link.href}
                  className={`text-[17px] font-medium hover:text-primary ${pathname === link.href ? 'text-primary' : 'text-[#1E1E1E]'}`}
            ...
            <div className="flex flex-col gap-3 mt-2">
              <Link
                href="https://app.interviewscreener.com/login"
                className="w-full py-2.5 text-center rounded-md border border-primary text-primary font-medium hover:bg-primary hover:text-white transition-colors"
              >
                Log In
              </Link>
              <Link
                href="https://app.interviewscreener.com/sign-up"
                className="w-full py-2.5 text-center rounded-[8.256px] bg-gradient-to-t from-[#00A0E2] via-[#3388FF] to-[#3388FF] text-white font-medium text-[16px] leading-[120%] tracking-[-0.48px] hover:opacity-90 transition-opacity"
              >
                Sign Up
              </Link>
            </div>
