    ...
    <div>
      ...
      <div className="mb-12">
        <div className="relative max-w-2xl mx-auto">
          <FaSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-text-light text-sm" />
          <input
            type="text"
            placeholder="Search articles by title, tags, or category..."
            value={searchQuery}
            onChange={(e) => handleSearch(e.target.value)}
            className="w-full pl-10 bg-white pr-4 py-3 rounded-lg border border-zinc-200 focus:border-primary focus:outline-none font-nunito text-base transition-colors"
          />
        </div>
        ...
        {searchQuery && (
          <p className="text-center mt-4 font-nunito text-sm text-text-light">
            ...
            {filteredArticles.length > 0 ? "s" : ""} found
          </p>
        )}
      </div>
          ...
          {paginationData.totalPages > 1 && (
            <div className="flex justify-center items-center gap-2">
              ...
              <button
                ...
                className="px-4 py-2 rounded-lg font-nunito font-medium text-sm transition-all duration-300 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-primary hover:text-white border border-zinc-200 hover:border-primary"
              >
                Previous
              </button>
                  ...
                  <button
                    ...
                    className={`w-10 h-10 rounded-lg font-nunito font-medium text-sm transition-all duration-300 ${
                      page === currentPage
                        ...
                        : "border border-zinc-200 hover:border-primary hover:bg-primary hover:text-white"
                    }`}
                  >
                    {page}
                  </button>
